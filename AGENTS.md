...
Estas instrucciones aplican a todo el repositorio. Este proyecto es el panel web de administración de MindSave: una SPA en español para autenticar administradores y consultar, crear, editar, desactivar y restaurar usuarios.
...
- React 19, TypeScript y Vite 8 con React Compiler habilitado.
...
- El acceso al panel exige el rol literal `PROFESIONAL_ROL`. El otro rol reconocido es `USER_ROL`. No cambies estos valores solo para mejorar etiquetas de UI.
...
- Centraliza llamadas HTTP en archivos `*.action.ts`. Devuelve datos tipados y pasa los errores de Axios por `handleError`; no muestres detalles sin normalizar ni ocultes fallos con datos ficticios.
